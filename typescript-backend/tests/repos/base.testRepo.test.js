...
describe("test the base repository", () => {
  let topTrx, trx;
  let todoDb, todoTestRepo, chosenTable;
  let session1, session1Id, sessions, assessmentId;
  ...
  beforeAll(async () => {
    ...
    sessions = await todoTestRepo.generateMany(3, {
      assessmentId,
      assessmentFormId: uuidV4(),
    });
  ...
  beforeEach(async () => {
    ...
    chosenTable = dbTables.sessions;

    //connecting it to *some* table for testing. Choosing 'sessions'
    todoRepository = createTodoListRepo(chosenTable, trx);
  });
